...
namespace NaviGoApi.Application.CQRS.Handlers.Shipment
{
	public class AddShipmentCommandHandler : IRequestHandler<AddShipmentCommand, Unit>
	{
		...
		public AddShipmentCommandHandler(IMapper mapper, IUnitOfWork unitOfWork, IHttpContextAccessor httpContextAccessor)
		{
			_mapper = mapper;
		...
		public async Task<Unit> Handle(AddShipmentCommand request, CancellationToken cancellationToken)
		{
			...
			if (user.UserRole != UserRole.CompanyAdmin)
				throw new ValidationException("You are not allowed to add shipment.");
			...
			if (user.CompanyId != company.Id)
				throw new ValidationException($"User cannot add shipment for wrong company.");
			...
			if (vehicle.CompanyId != company.Id)
				throw new ValidationException($"Vehicle with ID {vehicle.Id} doesn't belong to company with ID {company.Id}");
