...
{
	public class GetAllAvailableVehicleQueryHandler : IRequestHandler<GetAllAvailableVehicleQuery, IEnumerable<VehicleDto?>>
	{
		private readonly IUnitOfWork _unitOfWork;
		private readonly IMapper _mapper;
		private readonly IHttpContextAccessor _httpContextAccessor;

		public GetAllAvailableVehicleQueryHandler(IUnitOfWork unitOfWork, IMapper mapper, IHttpContextAccessor httpContextAccessor)
		{
			_unitOfWork = unitOfWork;
			_mapper = mapper;
			_httpContextAccessor = httpContextAccessor;
		}
		public async Task<IEnumerable<VehicleDto?>> Handle(GetAllAvailableVehicleQuery request, CancellationToken cancellationToken)
		{
			...
			if (userCompany.CompanyType != CompanyType.Carrier && userCompany.CompanyType != CompanyType.Forwarder)
				throw new ValidationException("Only Carrier companies can see vehicles.");
			IEnumerable<Domain.Entities.Vehicle> vehicles;
			vehicles = (await _unitOfWork.Vehicles.GetAvailableVehiclesAsync(userCompany.Id))
							.ToList();
			if (!vehicles.Any())
